...
/**
 * Aggressive HTML sanitiser. Strips every attribute from every element and
 * unwraps unknown tags so pasted Google Docs / Word content never carries
 * through visual formatting (white paragraph backgrounds, custom fonts).
 * Bold/italic/headings/lists survive because they're carried by tag names.
 */
 ...
 *  - starter:  free only (no monetisation contract yet)
 *  - partner:  free + premium (50/50 revenue share)
 *  - original: free + premium + privilege (curated Inkrupt Original)
 */
...
export default function StudioChapter() {
  ...
  useEffect(() => {
    ...
    pb.collection('chapters')
      ...
      .then((record) => {
        ...
        // Determine the saved type (with legacy is_premium fallback)
        const savedType: ChapterType =
          (record.type as ChapterType) || (record.is_premium ? 'premium' : 'free')

        // ── Auto-downgrade: if the saved type isn't allowed by the current
        // author tier, snap it back to Free and warn the author. This avoids
        // the confusing "selected + locked" state in the segmented control.
        let resolvedType: ChapterType = savedType
        if (!tierCanUseType(authorTier, savedType)) {
          resolvedType = 'free'
          setTimeout(() => {
            toast({
              title: `Tipo do capítulo ajustado para Grátis`,
              description: `Seu tier atual não permite "${savedType}". Para reverter, assine o contrato correspondente.`,
            })
          }, 100)
        }
  ...
  }, [chapterId, id, navigate, authorTier])
  ...
  const handleSave = async () => {
    ...
    try {
      const dataToSave: any = {
        ...formData,
        // Maintain backwards compat with legacy is_premium boolean
  ...
  return (
    // Fixed-height viewport container — children scroll internally, page never grows.
    <div className="h-[calc(100dvh-4rem)] flex flex-col overflow-hidden">
      <div className="container mx-auto px-4 max-w-5xl flex flex-col flex-1 min-h-0 py-4">

        {/* ────────────────────────── HEADER ROW 1 ──────────────────────────
            Back arrow • breadcrumb + title • Ler + Salvar */}
        <div className="flex items-center gap-3 shrink-0 pb-3 border-b">
        ...
        {/* ────────────────────────── HEADER ROW 2 ──────────────────────────
            Chapter type segmented control • coin price (if premium) • publish toggle */}
        <div className="flex items-center gap-3 shrink-0 py-3 flex-wrap">
          ...
          {formData.type === 'premium' && (
            <div className="flex items-center gap-2 bg-amber-500/5 px-3 py-2 rounded-xl border border-amber-500/20">
          ...
          <div className="flex items-center gap-3 bg-muted/50 px-4 py-2 rounded-xl border">
            ...
            <Switch
              ...
              onCheckedChange={(c) =>
                setFormData({ ...formData, status: c ? 'published' : 'draft' })
              }
            />
          </div>
        </div>

        {/* ─────────────────── Tier upgrade banner (starter only) ─────────────────── */}
        {authorTier === 'starter' && (
          <div className="mb-3 px-4 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-sm flex items-center gap-2 shrink-0">
        ...
        {errors.title && <p className="text-sm text-destructive mb-2 shrink-0">{errors.title}</p>}
        {errors.status && <p className="text-sm text-destructive mb-2 shrink-0">{errors.status}</p>}

        {/* ────────────────────────── EDITOR ──────────────────────────
            flex-1 + min-h-0 chain: this child takes all remaining vertical
            space and the editor's contentEditable scrolls inside it. */}
        <div className="flex-1 min-h-0 flex flex-col bg-card border rounded-2xl overflow-hidden shadow-sm">
          <RichTextEditor
        ...
        {errors.content && (
          <p className="text-sm text-destructive mt-2 shrink-0">{errors.content}</p>
        )}

        {/* ────────────────────────── FOOTER STATS ────────────────────────── */}
        <div className="mt-3 flex items-center justify-between text-sm text-muted-foreground shrink-0 border-t pt-3">
